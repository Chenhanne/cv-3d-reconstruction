...
def query(name):
    """
    :param name: 要查询的人物名称
    :return: 调用get_json_data自定义函数，返回
    方法内容：查询起始节点或终节点名称为 要查询人物 的关系 CQL
    """
    data = graph.run("match (p)-[r]->(n:Person{Name:'%s'}) return  p.Name, r.relation, n.Name, p.cate,n.cate\
        Union all\
...
def get_json_data(data):
    """
    将数据库中取出来的数据 格式化成JSON并返回
    :param data: 数据库取出来的关系 列表格式
    :return: 格式化后的JSON
    """
    json_data = {'data': [], "links": []}  # 字典
    d = []  # 列表
    for i in data:
        # print(i["p.Name"], i["r.relation"], i["n.Name"], i["p.cate"], i["n.cate"])
        d.append(i['p.Name']+"_"+i['p.cate'])
        d.append(i['n.Name']+"_"+i['n.cate'])  # 取出所有人物名称和类别
        d = list(set(d))  # 消除重复项
    name_dict = {}
    count = 0
    for j in d:
...
def get_answer_profile(name):
    with open("./spider/images/"+"%s.jpg" % (str(name)), "rb") as image:
        base64_data = base64.b64encode(image.read())
        b = str(base64_data)
    return [get_profile(str(name)), b.split("'")[1]]   # 使用的show_profile 里的方法 并且展示图片
